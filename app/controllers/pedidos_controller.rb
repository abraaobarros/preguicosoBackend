...
class PedidosController < ApplicationController
  ...
  # GET /pedidos.json
  def index
    @pedidos = Pedido.order("id DESC").all
    if @pedidos.empty?
      uri = URI.parse("http://4.preguicosotest.appspot.com/preguicoso/controller?class=Pedido&method=listAll&page=0&nPage=519")
      json = uri.open.read
      @json = JSON.parse(json)
      @json.shift
      @json.each do |j|
         item = Pedido.new
         item.id_estabelecimento = j["idEstabelecimento"]
         item.status = j["status"]
         item.lista_itens = j["itens"]
         item.id = j["id"]
         item.save
      end
    end
